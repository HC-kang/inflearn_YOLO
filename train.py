...
def calculate_loss(model, batch_image, batch_bbox, batch_labels):
  ...
  for batch_index in range(batch_image.shape[0]):
        # image 등의 데이터를 Process each ground truth 함수를 통해 배치 이미지 1장 단위로 데이터 재구성
  ...
  # 실제 한 스텝 실시, 갱신된, 현재 step에서 total loss -> optimizer
