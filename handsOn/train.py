...
voc2012_train_split_data = tfds.laod("voc/2012", split = tfds.Split.TRAIN, batch_size = 1)
...
def save_validation_result(model, ckpt, validation_summary_writer, num_visualize_image):
    ...
    for iter, features in enumerate(validation_data):
        ...
        validation_total_loss, validataion_coord_loss, validation_object_loss, validation_noobject_loss, validation_class_loss = calculate_loss(model, batch_validation_image, batch_validation_bbox, batch_validation_labels)
        
        total_validation_total_loss = total_validation_total_loss + validation_total_loss
        total_validation_coord_loss = total_validation_coord_loss + validataion_coord_loss        
    ...
    for validation_image_index in range(num_visualize_image):
        ...
        for each_object_num in range(object_num):
            ...
            if class_label == 7:
                ground_truth_bounding_box_info_list.append(yolo_format_to_bounding_box_dict(xcenter, ycenter, box_w, box_h, 'cat', 1.0))
                
        ground_truth_drawing_image = drawing_image.copy()
        # draw ground_truth image
        ...
        with validation_summary_writer.as_default():
            tf.summary.image('validation_iamge_' + str(validation_image_index), drawing_image, step = int(ckpt.step))
...
def main(_):
    ...
    if not os.path.exist(FLAGS.checkpoint_path):
    ...
    for epoch in range(FLAGS.num_epochs):
        ...
        for iter, features in enumerate(train_data):
            ...
            batch_labels = features['object']['label']
            ...
            if iter % FLAGS.validation_steps == 0:
                save_validation_result(YOLOv1_model, ckpt, validation_summary_writer, FLAGS.sum_visualize_image)
